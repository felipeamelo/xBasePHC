** PENDENCIAS
**-----------------
** Executar fScannerPOD (ID:52 do MyCargo) para importar os POD para Gestão Documental: \\192.168.1.174\Lusocargo\SERVICOS\POD\Scanner\*.*
** Executar fScannerCOM (ID:53, 54,55 do MyCargo) para importar os DOCs de Compras para Gestão Documental: \\192.168.1.174\Lusocargo\SERVICOS\POD\Scanner\Contabilidade\PHC - Sede\*.*


**//+-----------------------------------------------------------------------------------//
**//|Funcao....: fImportaPODdoFTP()
**//|Autor.....: Felipe Aurélio de Melo - melo.felipe@lusocargo.pt
...
**//Define o valor das variáveis conforme CLIENT FTP 
lOk = .T.
Do Case
    Case cClientFTP == "TFD"
        lcHostname  = "hostipserver.ddns.net"
        lnPort      = 21
        lcUserFtp   = "lusocargoporto"
        lcPassFtp   = "luso0777"
        ...
        lcDirNet2   = "\\192.168.1.174\Lusocargo\SERVICOS\POD\Nacional\Após 2021\"
    ...
    Case cClientFTP == "TBFILES_DISTRIB"
        ...
        lcDirLocal  = "\\192.168.1.174\Lusocargo\SERVICOS\POD\Distribuição (guias dos transportadores)\"
        lcDirTemp   = Alltrim(SYS(5)) + AddBS(CurDir()) + AddBS("Tmpfiles\TempPOD\")
        lcDirNet1   = ""
        lcDirNet2   = ""
    ...
    Case cClientFTP == "TBFILES_LCARGO_SUL"
        ...
        lcDirLocal  = "\\192.168.1.174\Lusocargo\SERVICOS\POD\Carga da LusocargoSul entregue por nós\"
        lcDirTemp   = Alltrim(SYS(5)) + AddBS(CurDir()) + AddBS("Tmpfiles\TempPOD\")
        lcDirNet1   = ""
        lcDirNet2   = ""
    ...
    Case cClientFTP == "TBFILES_NACIONAL"
        ...
        lcDirLocal  = "\\192.168.1.174\Lusocargo\SERVICOS\POD\Nacional\Após 2021\"
        lcDirTemp   = Alltrim(SYS(5)) + AddBS(CurDir()) + AddBS("Tmpfiles\TempPOD\")
        lcDirNet1   = ""
        lcDirNet2   = ""
    ...
    **//21: Conecta ao FTP
    ...
    If lOk
        ...
        **//21: Download POD via FTP
        fDwn21POD(@loSftp,.F.,cClientFTP,lcDirRemote,lcDirLocal,"5*.pdf",lcDirTemp,lcDirNet1,"")

        **//21: Download POD via FTP
        fDwn21POD(@loSftp,.F.,cClientFTP,lcDirRemote,lcDirNet2,"8*.pdf",lcDirTemp,"","")
        ...
        **//Apagar ficheiros da Pasta Temporários utilizado para manipular os ficheiros
...
If lOk And lnPort == 21 And cClientFTP == "LISBOA"
    **//21: Conecta ao FTP
    ...
    If lOk
        **//21: Download POD via FTP
        fDwn21POD(@loSftp,.F.,cClientFTP,lcDirRemote,@lcDirLocal,"LUSOCA*.pdf",lcDirTemp,lcDirNet1,lcDirNet2)
...
If lOk And lnPort == 21 And cClientFTP == "TBFILES_COMPRAS"
    **//21: Conecta ao FTP
    ...
    If lOk
        **//21: Download POD via FTP
        fDwn21POD(@loSftp,.T.,cClientFTP,lcDirRemote,@lcDirLocal,"*.*",lcDirTemp,lcDirNet1,lcDirNet2)

        **//Apagar ficheiros do FTP (Desativei essa função porque estou apagando o ficheiro do FTP logo apos baixar)
        **fDelArqFTP(@loSftp,lcDirRemote,"*.*",lcDirTemp,.T.)
...
If lOk And lnPort == 21 And cClientFTP == "TBFILES_LUSOCARGO"
    **//21: Conecta ao FTP
    ...
    If lOk
        ...
        **//Apagar ficheiros do FTP (Desativei essa função porque estou apagando o ficheiro do FTP logo apos baixar)
        **fDelArqFTP(@loSftp,lcDirRemote,"*.*",lcDirTemp,.F.)
...
If lOk And lnPort == 21 And cClientFTP == "TBFILES_LCARGO_SUL"
    **//21: Conecta ao FTP
    ...
    If lOk
        ...
        **//21: Download POD via FTP
        fDwn21POD(@loSftp,.T.,cClientFTP,lcDirRemote,@lcDirLocal,"*.*",lcDirTemp+'MAIA\',lcDirNet1,lcDirNet2)

        **//Apagar ficheiros do FTP (Desativei essa função porque estou apagando o ficheiro do FTP logo apos baixar)
        **fDelArqFTP(@loSftp,lcDirRemote,"*.*",lcDirTemp+'MAIA\',.T.)
...
If lOk And lnPort == 21 And cClientFTP == "TBFILES_PORTOCARGO"
    **//21: Conecta ao FTP
    ...
    If lOk
        ...
        **//21: Download POD via FTP
        fDwn21POD(@loSftp,.T.,cClientFTP,lcDirRemote,@lcDirLocal,"*.*",lcDirTemp+'PORTOCARGO\',lcDirNet1,lcDirNet2)

        **//Apagar ficheiros do FTP (Desativei essa função porque estou apagando o ficheiro do FTP logo apos baixar)
        **fDelArqFTP(@loSftp,lcDirRemote,"*.*",lcDirTemp+'PORTOCARGO\',.T.)
    ...
    For x=1 To nQtdDIRs
        **//Apaga Diretorios
        If AllTrim(Upper(StrTran(aArqLocal(x, 5),".",""))) == "D" And !(AllTrim(aArqLocal(x, 1)) $ ("\.\..\...\"))
            ...
            **//Busca informações para conectar no FTP
            If lOk
...
If nPort == 21
    **//Percorre todos os ficheiros da pasta e envia para o FTP
    For n = 1 To nTotalArquivos
        ...
        **//Enviado com sucesso, então pode apagar
        ...
        **//Enviado com sucesso, então pode apagar
        ...
        Else
            **//Grava LOG
            ...
            fGravaLog( 778, "fImportaPODdoFTP("+cClientFTP+")" ,cResumoLOG ,cMensagemLOG )
        EndIf
    Next n
EndIf

**//Fecha a janela de espera após o término do processamento
WAIT CLEAR
...
**//Caso Agente não tenha cadastro, registar no LOG
If lOk == .F. 
    **//Grava LOG
    cResumoLOG = "Agente ("+cCodAgente+") sem credenciais para FTP (POD) - [BDSQL.EDI_Lusocargo]"
    cMensagemLOG = "Não localizado o agente nas tabelas Credenciais128 ou Credenciais512"
    fGravaLog( 778, "fImportaPODdoFTP > fFindAcessFTP" ,cResumoLOG ,cMensagemLOG )
EndIf
...
**//+-----------------------------------------------------------------------------------//
**-----------------------------------------------------------*
Function fDelArqFTP(loSftp,lcDirRemote,lcFilenamePattern,lcDirTemp,lVerifDwn)
**-----------------------------------------------------------*
...
If lnSuccess == 1
    ...
    **//Apagando ficheiros FTP
    ...
    For x = 0 To nQtdeFiles - 1
        lOk = .T.
        **//Apenas ficheiros e NÃO sub-diretorios
        If loSftp.GetIsDirectory(x) <> 1
            **//Obtenha detalhes do Ficheiro
            lcRemoteFileName = AllTrim(loSftp.GetFilename(x)) && Nome do ficheiro

            If lVerifDwn
                **//Se for ficheiros foram realmente baixado antes de apagar
                If File(lcDirTemp+lcRemoteFileName)
...
For x=1 To nQtdPDFs
    ...
    **//Se arquivo temporário usado, fechar
    ...
    If !u_sqlexec(cTmpQry,"xTmpSQL")
        MENSAGEM("ERRO na leitura da guia. P.f. contacte a Informática."+Chr(13)+Chr(10)+"Ficheiro: "+cFileOrig,"DIRECTA")
        msg(cTmpQry)
    Else
        ...
        **//Se não tem referencia, então usar o numero da nossa reserva
...
* inicia a regua
REGUA(0, nRegistos, "Importando POD para Gestão Doc.")
...
For i = 1 To nRegistos
    ...
    If At("_", cFilePOD) > 0
        ...
        If !fIsDigit(cGuiaEntrega)
            texto = IdTarefa + "Importação de POD: Nome de ficheiro inválido."

            **//Grava LOG
            cResumoLOG = "ERRO - fScannerPOD("+cGuiaEntrega+") - Nome de ficheiro inválido."
    ...
    **//Verifica se query retorno resultado
    If Used("TempSQL") And Reccount("TempSQL") > 0
        ...
        Do While (!Eof("TempSQL"))
            ...
            **//Verifica de o numero da reserva e processo estão preenchidos
            ...
            **//Cria diretorio caso não exista
            If lOk And !Directory(cRedePOD + aStr(TempSQL.reserva))
                **//Tenta criar o diretório de destino
                Try
                    MkDir(cRedePOD + aStr(TempSQL.reserva))
                Catch To oErr
                    lOk = .F.
                    msg("Erro ao criar o diretório ("+cRedePOD + aStr(TempSQL.reserva)+")")
                EndTry
            EndIf
            
            **//Verifica se ficheiro já existe no servidor
            If lOk And File(cRedePOD + aStr(TempSQL.reserva) + "\" + cNewFilePOD)
                lOk = .F.

                **//Verifica tamanho do ficheiro, se for do mesmo tamanho, então mantem os dois ficheiros
                lLoop = .T.
                ...
                Do While lLoop And nTamFile != nTamNew
                    ...
                    **//Se ficheiro existe, verifica o tamanho 
                    If File(cRedePOD + aStr(TempSQL.reserva) + "\" + cNewFilePOD)
                        nTamNew = filesize(cRedePOD + aStr(TempSQL.reserva) + "\" + cNewFilePOD)
                        **//Se tamanho igual, então não gravar novamente
                        If nTamFile == nTamNew
                            lLoop = .F.
                        EndIf
                    Else
                        **//Se não encontrou, então sai do LOOP e grava
                        lLoop = .F.
                        lOk = .T.
                    EndIf
                ...
                If !lOk
                    **//Tratar MSG erro
                    msg("POD da Guia de Entrega já importado")
                EndIf
            EndIf
    ...
    Else
        **//Tratar MSG erro
        msg("Guia de Entrega não encontrada no PHC") 
    EndIf
...
* Usa ADIR para obter as informações do arquivo
If aDir(aArquivo, cArquivo) > 0
    * A terceira coluna contém a data, e a quarta coluna contém a hora
...
**-----------------------------------------------------------*
Local i, lIsDigit
lIsDigit = .T.  && Assume que a string é numérica

For i = 1 To Len(cString)
    If Not Between(SubStr(cString, i, 1), "0", "9")
        lIsDigit = .F.  && Encontrou um caractere não numérico
        Exit
    EndIf
EndFor
...
**//+-----------------------------------------------------------------------------------//
**-----------------------------------------------------------*
Function fDwn21POD(loSftp,lApagaFTP,cClientFTP,lcDirRemote,lcDirLocal,lcFilenamePattern,lcDirCopiaDest1,lcDirCopiaDest2,lcDirCopiaDest3)
**-----------------------------------------------------------*
                    ...
                    **// Tenta criar o diretório de destino
                    ...
                    Catch To oErr
                        ...
                        **//Grava LOG
                        ...
                        cMensagemLOG = "Erro ao criar o diretório de destino ("+lcNewDirLocal+"): " + oErr.Message
                        fGravaLog( 778, "fImportaPODdoFTP("+cClientFTP+")" ,cResumoLOG ,cMensagemLOG )
                        Return(lOk)
                    EndTry
                Endif
                ...
                ** Verificar se ficheiro existe nos nossos servidores
                If lOk
                    ...
                    Else
                        lOk = .F.
                        If lApagaFTP
                            **Apaga ficheiro da origem caso já tenha sido baixado antes
                            loSftp.DeleteRemoteFile(lcRemoteFileName)
                        EndIf
                    EndIf
                EndIf

                **Baixar somente ficheiros que não foram baixados anteriormente
                **Baixar apenas novos arquivos
                If lOk
                    Try
                        lLoop = .T.
                        nCount = 0
                        Do While lLoop and nCount < 3 
                            **Conta tentativas
                            nCount = nCount + 1
                            
                            **Verifica se ficheiro foi baixado com sucesso
                            If File(lcNewDirLocal+lcLocalFileName)
                               lLoop = .F.
                            Else
                        		**Download a file.
                    ...
                    Else
                        If lApagaFTP
                            **Apaga ficheiro da origem caso tenha baixado com sucesso
                            loSftp.DeleteRemoteFile(lcRemoteFileName)
                        EndIf
...
**//Copiar o arquivo
Do Case
    **//Se existe na origem e não existe no distino, então copia
    ...
    **//Se existe na origem e também existe no distino, então copia mas renomeria antes para não sobrepor
    Case File(cFileOrigPath) And File(cFileDestPath)
        **//Renomeia ficheiro
...
TEXT TO cTmpSQL TEXTMERGE NOSHOW
    ...
    and YEAR(dataimport) = YEAR(GETDATE())
ENDTEXT
...
**//Se for ficheiros da TBFiles sempre baixar, mesmo que repetido
If "TBFILES_COMPRAS" $ cClientFTP
    lOk = .T.
EndIf

**//Verifca se ficheiro existe no nosso servidor LOCAL
**//Se existir, verifica o tamanho do ficheiro e se for o mesmo, então não baixar novamente
